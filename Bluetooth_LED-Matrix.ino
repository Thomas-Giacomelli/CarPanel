...
#define APPO{B00110000,B00110000,B01100000,B00000000,B00000000,B00000000}
...
byte alphabet[][6]={BA,BB,BC,BD,BE,BF,BG,BH,BI,BJ,BK,BL,BM,BN,BO,BP,BQ,BR,BS,BT,BU,BV,BW,BX,BY,BZ,SPACE,NUM0,NUM1,NUM2,NUM3,NUM4,NUM5,NUM6,NUM7,NUM8,NUM9,DEVIDE,TWODOTS,DOT,COMA,LINE,QUASTION,MARK,APPO}; 
byte initPanel[2][6]={SPACE,SPACE};
...
byte make_word (long posistion,byte turn){
  ...
  void loop() {
    ...
    //Init panel
    display_word(1, initPanel,2,1);
              
    //Receive words to print
    ...
    if (content.length() > 0)
    {
      //Send to master the words to print, and the length of theim for user's verification
      Serial.println("string to leds : " + content + "  size : " + content.length());

      byte letterBits[content.length()][6];
      
      //Convert string to byte array
      for (compteurReception = 0; compteurReception < content.length(); compteurReception++)
      {
        int compteurBits = 0;
        
        //Return the index of the char in the byte array "alphabet"
        int indexLetter = findIndexLetter(content.charAt(compteurReception)); 
        
        for (compteurBits = 0; compteurBits < 6; compteurBits++)
        {
          //Stock chars to write in letterBits
          letterBits[compteurReception][compteurBits] = alphabet[indexLetter][compteurBits];
        }
      }
      
      display_word(1,letterBits,content.length(),15);// calls for the display_pattern function and says that int loop = 15(if you do more loop the pattern whould scrole slower).
      
      //Send a finish indicator to master
      Serial.println("String was written !");
    }
    
    content = "";
}
 
int findIndexLetter(byte letter)
{
  //SPACE
  if ((byte)letter == 0x20) //SPACE
  {
    return 26;
  }
  
  else if ((byte)letter == 0x21) //!
  {
    return 43;
  }  
  else if ((byte)letter == 0x3F) //?
  {
    return 42;
  }  
  else if ((byte)letter == 0x3A) //:
  {
    return 38;
  }  
    
  else if ((byte)letter == 0x2E) //.
  {
    return 39;
  }  
    
  else if ((byte)letter == 0x2C) //,
  {
    return 40;
  }  
  
  else if ((byte)letter == 0x27) //'
  {  
    return 44;
  }
  
  else if (((byte)letter >= (byte)'A') && ((byte)letter <=(byte)'Z') )
  {
    return (int)((byte)letter - (byte)'A');
  }
  
  else if (((byte) letter >= (byte)'0') &&((byte)letter <=(byte)'9') )
  {
    return 27 + ((byte) letter - (byte)'0');
  }
  
  else
  {
    return 26; //Return SPACE if unknown char
  }
}
